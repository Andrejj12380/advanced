...
select title, count(*) from albums a
left join tracks t on t.albumid = a.albumid
group by title
having count(*) = (select count from (select title, count(*) from albums a left join tracks t on t.albumid = a.albumid  group by title order by count(*) limit 1) as num);